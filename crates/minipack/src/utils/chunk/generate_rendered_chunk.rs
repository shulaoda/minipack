...
pub fn generate_pre_rendered_chunk(
  ...
  RollupPreRenderedChunk {
    ...
    facade_module_id: match &chunk.kind {
      ChunkKind::EntryPoint { module, .. } => Some(graph.modules[*module].id().to_string().into()),
      ChunkKind::Common => None,
    },
    module_ids: chunk.modules.iter().map(|id| graph.modules[*id].id().to_string().into()).collect(),
