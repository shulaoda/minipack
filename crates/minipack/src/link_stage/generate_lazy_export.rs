...
impl LinkStage<'_> {
  pub fn generate_lazy_export(&mut self) {
    ...
    for (ast_idx, is_json_module) in module_idx_to_exports_kind {
      ...
      let Some((ecma_ast, _)) = self.index_ecma_ast.get_mut(ast_idx) else { unreachable!() };
...
fn json_object_expr_to_esm(
  ...
  let (ecma_ast, _) = &mut link_staged.index_ecma_ast[ast_idx];
  ...
  let mut symbol_ref_db = SymbolRefDbForModule::new(module_idx, scoping, root_scope_id);
